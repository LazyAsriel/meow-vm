...
// ✂️ ĐÃ CẮT BỎ CÁC HẰNG SỐ CŨ (MEOW_EXP_MASK, MEOW_QNAN_POS...) ✂️

// Helper alias cho gọn
...
inline bool is_double(uint64_t b) { 
    // Dùng Layout::EXP_MASK thay vì MEOW_EXP_MASK
...
public:
    ...
    NaNBoxedVariant() noexcept {
        if constexpr (std::is_same_v<typename detail::nth_type<0, flat_list>::type, std::monostate>) {
            bits_ = Layout::QNAN_POS; // Thay MEOW_QNAN_POS
        } else {
            bits_ = Layout::VALUELESS; // Thay MEOW_VALUELESS
        }
    }
