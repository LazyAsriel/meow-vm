...
namespace meow::jit::runtime {
    extern "C" void jit_helper_binary_op_generic(int op, uint64_t v1, uint64_t v2, uint64_t* dst);
    extern "C" void jit_helper_get_prop_generic(uint64_t obj, uint64_t name, uint64_t* dst);
}
...
using Variant = meow::base_t; // base_t được định nghĩa public trong namespace meow

// Register Mapping
// R14: Pointer to Registers array
// R15: Pointer to Constants array
static constexpr Reg REG_VM_REGS_BASE = R14;
static constexpr Reg REG_CONSTS_BASE  = R15;

// Macros truy cập bộ nhớ
#define MEM_REG(idx)   REG_VM_REGS_BASE, (idx) * 8
#define MEM_CONST(idx) REG_CONSTS_BASE,  (idx) * 8

// --- Nanbox Constants Calculation ---
// Tính toán Tag tại Compile-time để hardcode vào Assembly
...
// Mask để lấy Tag (16 bit cao)
// Tag nằm ở bits [48..63]. 
// Để check Tag: (Value >> 48) == (TAG_INT >> 48)
...
Reg CodeGenerator::map_vm_reg(int vm_reg) const {
    // Map các thanh ghi ảo VM (0..2) vào thanh ghi vật lý CPU
    // Sử dụng Callee-saved registers để không bị clobber khi gọi hàm C++
    switch (vm_reg) {
        case 0: return RBX;
        case 1: return R12;
        case 2: return R13;
...
void CodeGenerator::emit_prologue() {
    ...
    // Setup VM Context
    // RDI = VMState*
    // VMState { Value* registers; Value* constants; ... }
    // Offset 0: registers, Offset 8: constants
...
Condition CodeGenerator::emit_cmp_logic(OpCode op_cmp, uint16_t r1, uint16_t r2) {
    ...
    // Note: So sánh raw bits (tốt cho Int/Bool/Null). 
    // Với Float hoặc String cần logic phức tạp hơn (unboxing).
...
JitFunc CodeGenerator::compile(const uint8_t* bytecode, size_t len) {
    ...
    while (ip < len) {
        ...
        auto emit_binary_op = [&](uint8_t opcode_alu, bool is_byte_op) {
            ...
            // 2. Type Check (Tag Check)
            // Logic: (Value >> 48) == (TAG_INT >> 48)
            
            // Check R1
            asm_.mov(R8, r1_reg);
            asm_.sar(R8, TAG_SHIFT); // Shift phải để lấy Tag
            // So sánh với hằng số Tag Int (đã shift)
            // Vì TAG_CHECK_VAL nhỏ (0x7FFA...), có thể vừa imm32? 
            // 0x7FFA > 32 bit signed. Cần load vào register.
            ...
            // Check R2 (R9 vẫn đang giữ TAG_CHECK_VAL)
            asm_.mov(R8, r2_reg);
            ...
            // 3. Fast Path (Unbox -> ALU -> Rebox)
            // Unbox: Sign-extend 48-bit Payload -> 64-bit Int
            // SHL 16, SAR 16
            
            asm_.mov(R8, r1_reg);
            asm_.shl(R8, 16);
            asm_.sar(R8, 16); // R8 = raw int1
            
            asm_.mov(R9, r2_reg);
            asm_.shl(R9, 16);
            asm_.sar(R9, 16); // R9 = raw int2
            ...
            // Rebox: Mask Payload & OR Tag
            // Result is in R8. 
            // Cần đảm bảo kết quả nằm trong 48 bit? (Overflow check?)
            // Tạm thời giả định fit.
            asm_.mov(R9, Layout::PAYLOAD_MASK);
            asm_.and_(R8, R9); // Clear trash bits
            
            asm_.mov(R9, TAG_INT);
            asm_.or_(R8, R9); // Apply Tag
        ...
        switch (op) {
            ...
            case OpCode::JUMP_IF_FALSE: case OpCode::JUMP_IF_FALSE_B: {
                ...
                // Check False: Cần check TAG_BOOL và value == 0 (false)
                // Hoặc check NULL?
                // Tạm thời dùng test r, r (giả sử False/Null encoded là 0 hoặc tương tự)
                // TODO: Implement chính xác check False theo Variant
                asm_.test(r, r); 
    ...
    for (auto& sp : slow_paths_) {
        ...
        // 2. Setup Runtime Call (System V ABI)
        ...
        // Call helper
        asm_.mov(RAX, (uint64_t)&runtime::jit_helper_binary_op_generic);
        asm_.call(RAX);

        // Reload cached reg if needed
