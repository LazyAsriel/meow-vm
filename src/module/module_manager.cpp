 ...
 * @author LazyPaws
 * @brief Implementation of Module Manager (Fixed Segfault during load)
 */
...
// [QUAN TRỌNG] Include guard để tắt GC tạm thời
#include <meow/memory/gc_disable_guard.h> 

namespace meow {

// Helper đệ quy để gắn Module chủ quản cho tất cả Proto con
...
module_t ModuleManager::load_module(string_t module_path_obj, string_t importer_path_obj) {
    ...
    // --- A. Thử tải Native Module (.dll/.so) ---
    ...
    if (!resolved_native_path.empty()) {
        ...
        // Check cache lần nữa với đường dẫn tuyệt đối
        ...
        if (!symbol) {
            ...
            throw std::runtime_error("Không tìm thấy symbol '" + std::string(factory_symbol_name) + "'");
        }
        ...
        try {
            ...
            if (!native_module) throw std::runtime_error("Native factory returned null.");
            native_module->set_executed();
        } catch (const std::exception& e) {
            close_native_library(handle);
            throw std::runtime_error("Lỗi khởi tạo module native: " + std::string(e.what()));
        }
    ...
    // --- B. Thử tải Bytecode Module (.meowb) ---
    ...
    if (!file.is_open()) {
        throw std::runtime_error("Không thể tìm thấy module (đã thử native và bytecode '" + 
                                 binary_file_path + "')");
    }
    ...
    if (!file.read(reinterpret_cast<char*>(buffer.data()), size)) {
         throw std::runtime_error("Lỗi đọc file: " + binary_file_path);
    }
    file.close();

    // [FIX] Tắt GC trong scope này để bảo vệ các object đang được tạo
    // Nếu không có cái này, GC có thể chạy giữa chừng và xóa object của Loader -> Segfault
    {
        ...
        try {
            ...
            throw std::runtime_error("Bytecode hỏng: " + binary_file_path + " - " + e.what());
        }
        ...
        // Link phase (tối ưu hóa bytecode)
        Loader::link_module(meow_module);

        // Gắn module owner cho các proto
